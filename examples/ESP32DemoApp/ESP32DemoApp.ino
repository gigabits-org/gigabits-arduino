...
// Initialize the Light sensor.  We need to turn the power on and
// tell it how long to integrate each sammple
// https://github.com/ControlEverythingCommunity/TSL2561/blob/master/Arduino/TSL2561.ino
void setupTSL() {
  // Start I2C Transmission
...
void sendTSLData() {
  Serial.println("Sending visible and infrared ...");
  unsigned int data[4];
  for(int i = 0; i < 4; i++)
  {
  ...
  Serial.print("Visible Light: ");Serial.println(ch0 - ch1);
  gigabits.sendRecord(VISIBLE_LIGHT_SENSOR_IDX, ch0 - ch1);
  Serial.print("Infrared Light: ");Serial.println(ch1);
